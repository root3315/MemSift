...
class NetworkAnalyzer(AnalysisPlugin):
    ...
    # Private/reserved IP ranges that shouldn't appear in certain contexts
    PRIVATE_PATTERNS = [
    ...
    C2_PATTERNS = [
        ...
        re.compile(r'(?i)callback'),
        re.compile(r'(?i)stage[0-9]*'),
        re.compile(r'(?i)payload'),
        re.compile(r'(?i)shell'),
        re.compile(r'(?i)c2[_-]?server'),
        re.compile(r'(?i)command[_-]?control'),
    ]
    ...
    def _analyze_string(self, string: str, offset: int) -> list[NetworkArtifact]:
        ...
        # Check for domains
        domain_match = re.search(r'\b([a-zA-Z0-9][-a-zA-Z0-9]*\.[a-zA-Z]{2,})\b', string)
        if domain_match and not ip_match:  # Avoid double-counting IPs
        ...
        # Check for port patterns (e.g., :8080)
    ...
    def _check_ip_suspicion(self, artifact: NetworkArtifact) -> None:
        ...
        # Check for localhost/loopback in suspicious context
        if ip.startswith('127.'):
            return  # Usually not suspicious
        
        # Check for private IPs in potentially suspicious contexts
    ...
    def _check_url_suspicion(self, artifact: NetworkArtifact) -> None:
        ...
        suspicious_paths = ['/gate.php', '/panel/', '/bot/', '/cmd/', '/beacon/']
    ...
    def _create_finding(self, artifact: NetworkArtifact) -> AnalysisFinding:
        ...
        if "malicious" in " ".join(artifact.suspicion_reasons).lower():
            severity = "high"
        elif "c2" in artifact.artifact_type:
            severity = "high"
        ...
        return AnalysisFinding(
            ...
            description=f"Detected suspicious {artifact.artifact_type}: {artifact.value}. "
    ...
    def _create_frequent_ip_finding(self, ip: str, count: int) -> AnalysisFinding:
        ...
        return AnalysisFinding(
            ...
            description=f"IP address {ip} appeared {count} times in memory. "
    ...
    def get_statistics(self) -> dict:
        ...
        return {
            ...
            'artifact_types': dict(Counter(a.artifact_type for a in self._artifacts)),
        }
